...
import nc.pub.pp.rept.execprice.constant.ExecPriceQueryConst;
import nc.scmmm.ui.scmpub.report.action.SCMRptQueryConditionDLGInitializer;
import nc.ui.pp.rept.execprice.action.ExecPriceValidate;
...
public class FBPuCtRptRefInitialzer extends
...
*/
private void filterDept(QueryConditionDLGDelegator delegator, String deptcode) {
QDeptFilter deptFilter =
    QDeptFilter.createQDeptFilterOfPU(delegator, deptcode);
...
QMarterialFilter materialFilter =
    new QMarterialFilter(delegator, this.getMainOrgField(), fieldcode);
...
private void filterMaterialoid(QueryConditionDLGDelegator delegator,
  String fieldcode) {
QMarterialoidFilter materialFilter =
    new QMarterialoidFilter(delegator, this.getMainOrgField(), fieldcode);
materialFilter.setbDiscount(UFBoolean.FALSE);
materialFilter.setbFee(UFBoolean.FALSE);
materialFilter.filter();
materialFilter.addEditorListener();
...
private void filterPsn(QueryConditionDLGDelegator delegator, String fieldcode) {
QPsndocFilter bizid =
    QPsndocFilter.createQPsndocFilterOfPU(delegator, fieldcode);
...
condDLGDelegator.registerICriteriaEditorValidator(new ExecPriceValidate());
}
